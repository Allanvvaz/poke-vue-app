<template>
    <div class="species-container">
      ...
      <div v-if="species">
        <h2>{{ species.name }} <small>#{{ species.id }}</small></h2>
        ...
        <div class="info-section">
          ...
          <p>{{ species.genera.find(g => g.language.name === 'en')?.genus || 'Unknown' }}</p>
        </div>
  
        <div class="info-section">
          <h3>Flags</h3>
          <p><strong>Is Baby:</strong> {{ species.is_baby ? 'Yes' : 'No' }}</p>
          <p><strong>Is Legendary:</strong> {{ species.is_legendary ? 'Yes' : 'No' }}</p>
          <p><strong>Is Mythical:</strong> {{ species.is_mythical ? 'Yes' : 'No' }}</p>
        </div>
