export default {
    ...
    async fetchPokemonTypes({ commit }) {
      try {
        ...
        const data = await response.json();
        commit('SET_POKEMON_TYPES', data.results.map(type => type.name));
      } catch (error) {
    ...
    async fetchPokemonByType({ commit, state, dispatch }) {
      ...
      if (state.typePagination.loading || 
          (state.typePagination.offset >= state.typePagination.total && state.typePagination.total > 0)) {
        return;
      }
      ...
      try {
        ...
        const data = await response.json();
        commit('UPDATE_TYPE_PAGINATION', { key: 'total', value: data.pokemon.length });
        ...
        commit('SET_POKEMON_LIST_BY_TYPE', state.pokemonList.map(p => p.id));
        ...
        if (state.currentTypeIndex === getters.pokemonListFiltrada.length - 1 && 
          ...
          return dispatch('fetchPokemonByType').then(() => {
            if (state.currentTypeIndex < getters.pokemonListFiltrada.length - 1) {
              ...
              dispatch('fetchPokemon', getters.pokemonListFiltrada[state.currentTypeIndex].id);
            }
          });
        } else if (state.currentTypeIndex < getters.pokemonListFiltrada.length - 1) {
          commit('SET_CURRENT_TYPE_INDEX', state.currentTypeIndex + 1);
          dispatch('fetchPokemon', getters.pokemonListFiltrada[state.currentTypeIndex].id);
        }
      } else {
